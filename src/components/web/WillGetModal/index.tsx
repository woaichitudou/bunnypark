...
export const Components: FC<Porps> = ({ show = false, onClose, onClick, dollar, bg, serviceFee, usdt }) => {
  return (
    <C.WebModal show={show} onClose={onClose} size="large">
      <div className="web-will-get-modal">
        <div className="web-will-get-modal-title">You will get</div>
        <div className="web-will-get-modal-icon">
          <C.WebImage src='adventure/avar.png' />
          <div className="web-will-get-modal-icon-right">
            <span>{securityInterception(bg as any, 6)} BG</span>
            <small>≈${dollar}</small>
          </div>
        </div>
        <div className="web-will-get-modal-sever">Withdrawal Fees：<span>{securityInterception(serviceFee as any, 3)}</span>%</div>
        <div className="web-will-get-modal-total">
          Wallet Balance:
          <span>{securityInterception(usdt as any, 6)} BG</span>
        </div>
        <C.WebButton type='blue' onClick={onClick}>Confirm</C.WebButton>
      </div>
    </C.WebModal>
  )
}
