...
export const Components: FC<Porps> = ({ show = false, onClose, onClick, entranceFee }) => {
  ...
  return (
    <C.WebModal show={show} onClose={onClose} size="large">
      <div className="web-pay-tickets-modal">
        <div className="web-pay-tickets-modal-title">Air Ticket</div>
        ...
        <div className="web-pay-tickets-payment">
          <p>Select payment method</p>
          <div className="web-pay-tickets-payment-box">
            {paymentData.map((item, index) => {
              return (<div key={index}
                className={classnames('web-pay-tickets-payment-box-min', { 'web-pay-tickets-payment-box-min-active': item.key === payment })}
                onClick={() => {
                  setPayment(item.key)
                }}>{item.name}</div>)
            })}
        ...
        <div className="web-pay-tickets-modal-total">
          Wallet Balance:<label>{securityInterception(switchBalance ?? 0, 3)}</label> {payment}
        </div>
        <C.WebButton type='blue' disabled={isDisabled} onClick={() => onClick?.(payment)}>Confirm</C.WebButton>
      </div>
    </C.WebModal>
  )
}
