...
const FleetCard: FC<any> = ({ seat = 2, handleSend = () => null, flips }) => { // flips 卡片翻转就需要传flips部分地方不需要翻转所以多加了参数
  ...
  return (<div className='m-c-card-shipcard-box'>
    {!showRear ? <div onClick={() => { onFlip() }} className={classNames('m-c-card-shipcard', `m-c-card-shipcard-seat-${seat}`)}>
      <h4>{name}</h4>
    </div> : <div className='m-c-card-shipcard-box-rear' onClick={() => { onClose() }}>
