...
const Upgrade: FC = () => {
  ...
  const handleBuy = () => {
    ...
    history.push(`/m/bunny/upgrade/pay/${tokenId}/${levelPower}/${quantity}`)
  }

  return (
    <div className='m-rabbit-upgrade'>
      <C.BackButton />
      ...
      <div className='m-rabbit-upgrade-card'>
        {
      ...
      <div className='m-rabbit-upgrade-step'>
        <span>Level</span>
        <C.Stepper min={1} max={max} value={quantity} onChange={(val: number) => setQuantity(val)}/>
      </div>
      <div className='m-rabbit-upgrade-btn'><C.Button disabled={!isCanbuy} onClick={handleBuy}>Buy Now</C.Button></div>
    </div>
  )
}
