...
const UpgradeChoose: FC = () => {
  ...
  const handleUpgrade = useCallback(async () => { // 升级
    ...
    try {
      ...
      await setStatus('')
      await setShow(false)
      history.push(`/m/bunny/upgrade/succeed/${tokenId}`)
    } catch (e) {
      setStatus('Failure')
    }
  ...
  return (
    <div className='m-rabbit-upgrade-choose'>
      <C.BackButton />
      <C.Title title='Upgrade Combat Power' />
      <p className='m-rabbit-upgrade-choose-title'>Choose payment method</p>
      <div className="m-gas-payment-box m-rabbit-upgrade-choose-box">
        {paymentData.map((item, index) => {
          return (<div key={index}
            className={classnames('m-gas-payment-box-min', { 'm-gas-payment-box-min-active': item.value === payWay })} onClick={() => setPayWay(item.value)}>{item.name}</div>)
        })}
      </div>
      <p className='m-rabbit-upgrade-choose-label'>Upgrade {quantity}  level{quantity > 1 ? 's' : ''} with <span>{usdtAmount}</span> USDT and <span>{payAmount}</span> {payWay === 1 ? 'BG' : 'BP'}.</p>
      <div className="m-rabbit-upgrade-choose-balance">
      ...
      <div className="m-rabbit-upgrade-choose-btn">
        <C.Button disabled={!usdtAmount} onClick={() => { setShow(true) }}>Upgrade</C.Button>
      </div>
      <C.UpgradePayModal onClick={handleUpgrade} show={show} onClose={() => { setShow(false) }} usdtAmount={usdtAmount} usdtBalance={USDTBalance} currencyAmount={payAmount} currencySymbol={payWay === 1 ? 'BG' : 'BP'} currencyBalance={payWay === 1 ? BGBalance : BPBalance}></C.UpgradePayModal>
