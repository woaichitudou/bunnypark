...
const NoData: FC = () => {
  ...
  return (
    <div className="web-header-fleet-no-data">
      <C.WebImage src='fleet/nodata.png' />
      <span>NO NFT CARD</span>
      <C.Button onClick={() => history.push('/spaceship')}>Build a fleet</C.Button>
    </div>
  )
}
...
const Fleet: FC = () => {
  ...
  const FleetCards = useMemo(() => {
    ...
    return (<div className="web-header-fleet-content">
      {
        fleetsInfo.map((fleet) => {
          ...
          return (
            <div key={fleet.tokenId} className="web-header-fleet-content-box">
              <FleetCard
                status={status}
                oil={fleet.gas}
                luck={(fleet.buff) / 10}
              ...
              <C.WebButton
                ...
                onClick={() => history.push(`/fleet/${fleet.tokenId}/driving-mode?power=${fleet.power}`)}>Adventure</C.WebButton>
            </div>)
        })
      }
    </div>)
  ...
  return (
    <div className='web-header-fleet'>
      <C.WebTitle title='Fleet' />
      { FleetCards }
      <C.WebSendCard
        cardType='fleet'
