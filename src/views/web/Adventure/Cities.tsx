...
const Cities: FC = () => {
  ...
  const handleAdventureEv = useCallback((cityId: number, power: number) => {
    history.push(`/adventure/cities/${cityId}/capacity/${power}/fleet`)
  }, [history])
  ...
  return (
    <div className='web-cities'>
      <div className="web-cities-dashboard">
        ...
        <C.WebButton type='blue' disabled={isDisabled} onClick={() => setModalStatus('confirm')}>Withdraw</C.WebButton>
      </div>
      <div className="web-cities-list">
        {CitiesCards}
      </div>
