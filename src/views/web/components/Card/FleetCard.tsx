...
const CardRear: FC<any> = ({ onClose, isSalable, handleSend, handleDetail, handleBuyFuel }) => {
  ...
  return (
    <div className="web-c-card-fleetcard-rear">
      <p>
      ...
      <div className="web-c-card-fleetcard-rear-mask" onClick={onClose} />
    </div>
  )
}
...
const ShipCard: FC<any> = ({
  ...
  return (
    <div className={classNames('web-c-card-fleetcard', `${status ? `web-c-card-fleetcard-status-${status}` : ''}`, `${size ? `web-c-card-fleetcard-${size}` : ''}`)}>
      {
        !showRear
          ? (<>
            <label onClick={goShowRear} />
            {
              status === 'rest' && <div className='web-c-card-fleetcard-time'><C.WebImage src='fleet/sleep-clock.png' /><span>{cutDownTime}</span></div>
            }
            <div className="web-c-card-fleetcard-inset" onClick={goShowRear}>
              <div className="web-c-card-fleetcard-image">
                <C.WebImage src={card.img} />
              </div>
              <h4>{card.name}</h4>
              <ul className="web-c-card-fleetcard-info">
                <li>{tokenId}</li>
                <li>{oil}</li>
                <li>{power}</li>
                <li>{luck}%</li>
