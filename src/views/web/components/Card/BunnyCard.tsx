...
const CardLevel: FC<any> = ({ maxLevel, level }) => {
  ...
  return (
    <div className="web-c-card-level">
      <span>{level * 100}</span>
      {
...
const CardRear: FC<any> = ({ tokenId, onClose, handleSend, isUpgrade, type }) => {
  ...
  const handleUpgrade = () => {
    history.push(`/bunny/upgrade/${tokenId}`)
  ...
  return (
    <div className="web-c-card-inset-rear">
      <p>
      ...
      <div className="web-c-card-inset-rear-mask" onClick={onClose} />
    </div>
  )
}
...
const BunnyCard: FC<any> = ({
  ...
  if (type === 'bunny') {
    ...
    return (
      <div className={classNames('web-c-card-bunnycard', `web-c-card-size-${size}`)}>
        <div className="web-c-card-inset">
          {
            !showRear
              ? (
                <div className="web-c-card-inset-front" onClick={goShowRear}>
                  <div className="web-c-card-image">
                    <C.WebImage src={cardInfo.img} />
                  </div>
                  <div className={classNames('web-c-card-info', `web-c-card-info-${rank}`)}>
                    <h4>{cardInfo.name}</h4>
                    <C.WebCopySpanner tokenId={tokenId} />
                  </div>
                  {
              ...
              ? (
                <div className="web-c-card-inset-front" onClick={goShowRear}>
                  ...
                  <div className="web-c-card-info">
                    <h4>BabyBunny</h4>
                    <div className="web-c-card-info-luck">0.5%</div>
                  </div>
                </div>
              )
